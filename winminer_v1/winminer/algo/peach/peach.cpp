...
int peach(BTRAILER *bt, word32 difficulty, word32 *hps, int mode)
{
   ...
   printf("Peach mode %i, diff %i\n", mode, diff);   
   if(mode == 0) {
      /* Allocate MAP on the Heap */
      map = (byte*)malloc(MAP_LENGTH);
      if(map == NULL) {
         printf("Fatal: Unable to allocate memory for map.\n");
         goto out;
      }
      memset(map, 0, MAP_LENGTH);
  
      /* Allocate MAP cache on the Heap */
      cache = (byte*)malloc(MAP);
      if(cache == NULL) {
         printf("Fatal: Unable to allocate memory for cache.\n");
         goto out;
      }
      memset(cache, 0, MAP);
   } else {
      tile = (byte*)malloc(TILE_LENGTH);
      if(tile == NULL) {
         printf("Fatal: Unable to allocate memory for tile.\n");
         goto out;
      }
   }
   ...
   for(;;) {
      ...
      if(solved) { /* We're Mining & We Solved! */
         if(peach(bt, difficulty, NULL, 1)) {
            ...
            printf("!!!!!Peach Validation failed IN THE CONTEXT!!!!!");
            printf("BT -> %s", hex);
         }
         ...
         *hps = h;
         ...
         char haiku[256];
         trigg_expand2(bt->nonce, (byte*)haiku);
         printf("\nS:%s\n\n", haiku);
...
out:
   ...
   if(mode == 1 && solved == 0) printf("?????Peach Validation failed?????");
