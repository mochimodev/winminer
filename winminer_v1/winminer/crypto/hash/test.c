...
static void test_blake2b_wo_key()
{
    ...
    for (int j = 0; j< 3; j++)
    {
		...
		BYTE *blake2b_inp = (BYTE*)malloc(INPUT_SIZE * 1024 * sizeof(BYTE));
		BYTE *blake2b_oup = (BYTE*)malloc(1024 * BLAKE2B_BLOCK_SIZE * sizeof(BYTE));
        srand(0);
...
static void test_keccak_w_key()
{
    ...
    for (int j = 0; j< 3; j++)
    {
        ...
        // TEST KECCAK
		BYTE *keccak_inp = (BYTE*)malloc(INPUT_SIZE * 1024 * sizeof(BYTE));
		BYTE *keccak_oup = (BYTE*)malloc(1024 * KECCAK_BLOCK_SIZE * sizeof(BYTE));
        srand(0);
        for (unsigned int i = 0; i < 1024 * INPUT_SIZE; i++)
        {
		...
		BYTE *keccak_cu_oup = (BYTE*)malloc(1024 * KECCAK_BLOCK_SIZE * sizeof(BYTE));
