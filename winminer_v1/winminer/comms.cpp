...
SOCKET connectip(word32 ip)
{
	...
	printf("\nTrying %s port %d...  ", ntoa((byte *)&ip), port);
...
retry:
	if (connect(sd, (struct sockaddr *) &addr, sizeof(struct sockaddr))) {
		...
		closesocket(sd);
		printf("\nconnectip(): cannot connect() socket.");
		return INVALID_SOCKET;
	}
out:
	printf("connected.");
...
int callserver(NODE *np, word32 ip)
{
	...
	if (ip) { Coreplist[0] = ip;  Nextcore = 0; }
	np->sd = INVALID_SOCKET;

	for (j = 0; j < CORELISTLEN && Sigint == 0; j++, Nextcore++) {
...
int get_cblock(word32 ip, char *fname)
{
	...
	if (cmp64(ServerBnum, tempCblocknum) < 0) {
		printf("\nThis server doesn't have the latest block.  Try a different server.");
		closesocket(node.sd);
		Nextcore++;
		return VERROR;
	}
